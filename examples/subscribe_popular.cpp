...
int main(int argc, char **argv) {
    ...
    if (username.empty() || password.empty() || account_id.empty()) {
        std::cerr << "Error: Username, password, and account ID are required" << std::endl;
        print_usage(argv[0]);
        return 1;
    }

    try {
        td365 client([](const tick& t) {
          std::cout << t << std::endl;
        });
        ...
        auto markets = client.get_market_quote(super_groups.at(0).id);
        client.subscribe(markets[0].quote_id);
