...
ws_client::ws_client(const user_callbacks &callbacks)
    : ctx_(ctx), ws_(std::make_unique<ws>(ctx)),
      auth_future_(auth_promise_.get_future()),
      disconnect_future_(disconnect_promise_.get_future()) {}

ws_client::~ws_client() {}

void ws_client::start_loop(std::string host, std::string login_id,
                           std::string token) {
  net::co_spawn(
      ctx_.executor,
      [this, token, login_id, host]() -> net::awaitable<void> {
        try {
          ...
          std::println("ws_client: message loop exiting: {}",
                       ctx_.is_shutting_down());
...
ws_client::process_messages(const std::string &login_id,
  ...
  while (!ctx_.is_shutting_down()) {
    ...
    if (ec) {
      std::println(std::cerr, "ws_client: error reading message: {}",
                   ec.message());
...
ws_client::process_authentication_response(const nlohmann::json &msg) {
  ...
  if (!connection_id_.empty()) {
    co_await send(
        {{"action", "reconnect"}, {"originalConnectionId", connection_id_}});
  }
  connection_id_ = msg["cid"].get<std::string>();
