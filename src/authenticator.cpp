...
authenticator::authenticator(std::string username, std::string password,
  ...
  assert(!username_.empty() && !password_.empty() && !account_id_.empty());
}

boost::asio::awaitable<account_detail>
authenticator::authenticate(const boost::asio::any_io_executor &executor) {
  auto token = auth_token::load();
  if (std::chrono::system_clock::now() > token.expiry_time) {
    token = co_await login(executor, username_, password_);
    token.save();
  }
  ...
  auto account = co_await select_account(client, account_id_);
