    ...
    tick tick::parse(const std::string_view line) {
        ...
        if (idx != EXPECTED_FIELDS) {
            throw std::invalid_argument(
                "Invalid CSV tick format: expected " + std::to_string(EXPECTED_FIELDS) + " fields, got " +
                std::to_string(idx));
        }
        ...
        auto parse_double = [](std::string_view sv) -> double {
            double value{};
            auto result = boost::charconv::from_chars(sv.data(), sv.data() + sv.size(), value);
        ...
        auto parse_bool = [](std::string_view sv) -> bool {
            return sv == "true";
        };
